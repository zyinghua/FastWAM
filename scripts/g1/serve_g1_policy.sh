...
Examples (from the FastWAM repository):
  ...
  bash scripts/g1/serve_g1_policy.sh /models/g1.pt --smoothness-dir /workspace/FastWAM/action_traces
...
--smoothness-dir records denormalized predicted execution prefixes and replan
boundaries; it does not confirm robot execution. --smoothness-method overrides
the automatic Fast-WAM, Joint-WAM, or IDM-WAM inference label.
